#!/usr/bin/env python3
"""Waypoint moveTo e2e against the local Carpet test server."""
    ...
    with rcon:
        ...
        action = Action.create(
            "moveTo",
            {
                ...
                "arrival_radius": 0.85,
                "timeout_ticks": 500,
                "no_progress_ticks": 80,
                "max_deviation": 5.0,
            },
        )
        accepted = body.execute(action)
        if not accepted.ok or not accepted.accepted:
            raise AssertionError(f"waypoint moveTo was not accepted: {accepted}")

        terminal = body.await_action_terminal(action.id, timeout_s=25.0)
        ...
        if not terminal.data.get("arrived"):
            raise AssertionError(f"waypoint moveDone did not report arrived: {terminal}")
        if int(terminal.data.get("waypoint_count", 0)) != len(waypoints):
            raise AssertionError(f"waypoint_count not reported correctly: {terminal}")
        if dist > 1.25:
