...
class NavigationConsumerSourceTests(unittest.TestCase):
    ...
    def test_block_work_consumers_use_shared_navigation_entrypoints(self):
        ...
        self.assertIn("self.egress_to_dry(", surface)
        egress = function_body(BLOCK_WORK, "egress_to_dry")
        self.assertIn("_find_surface_egress_domain(", egress)
        self.assertIn("self.navigator.navigate_to(", egress)
        self.assertIn("_find_lateral_surface_domain(", surface)
        self.assertIn("GoalComposite(", surface)
        self.assertEqual(surface.count("self.navigator.navigate_to("), 1)
        self.assertEqual(egress.count("self.navigator.navigate_to("), 1)
        self.assertIn("aquatic_navigation_config(", egress)
        self.assertNotIn("pure_movement_navigation_config(", egress)
