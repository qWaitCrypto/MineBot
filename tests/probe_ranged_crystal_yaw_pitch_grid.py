#!/usr/bin/env python3
"""Focused live yaw/pitch grid for fake-player bow shots against a supported crystal."""
...
YAWS = [-6, -4, -2, 0, 2, 4, 6]
PITCHES = [-20, -19, -18, -17, -16, -15, -14, -13, -12]
...
def setup_world(rcon) -> None:
    ...
    for cmd in [
        ...
        f"fill {x-5} {y} {z-5} {x+5} {y+8} {z+24} air",
        f"fill {x-5} {y-1} {z-5} {x+5} {y-1} {z+24} stone",
    ...
    for cmd in [
        ...
        f"item replace entity {BOT} weapon.offhand with arrow 64",
...
def shoot_once(rcon, yaw: int, pitch: int) -> dict[str, object]:
    command(rcon, "kill @e[type=arrow]", delay=0.02)
    command(rcon, f"tp {BOT} {BASE[0]} {BASE[1]} {BASE[2]} 0 0", delay=0.02)
    command(rcon, f"player {BOT} look {pitch} {yaw}", delay=0.05)
    ...
    with connect_or_skip() as rcon:
        for yaw in YAWS:
            for pitch in PITCHES:
                setup_world(rcon)
                result = shoot_once(rcon, yaw, pitch)
